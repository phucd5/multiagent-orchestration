...
class SpecialistsOrchestrator(BaseOrchestrator):
    """
    Orchestrator for the specialists team.
    Manager coordinates PM, TL, SWE, and QA agents.
    """
    ...
    def __init__(
        ...
        self.base_manager_prompt = self._load_prompt(
            "orchestrators/prompts/specialists/sys_prompt_manager.md"
        )
        self.agent_prompts = {
            "pm": self._load_prompt(
                "orchestrators/prompts/specialists/sys_prompt_pm.md"
            ),
            "tl": self._load_prompt(
                "orchestrators/prompts/specialists/sys_prompt_tl.md"
            ),
            "swe": self._load_prompt(
                "orchestrators/prompts/specialists/sys_prompt_swe.md"
            ),
            "qa": self._load_prompt(
                "orchestrators/prompts/specialists/sys_prompt_qa.md"
            ),
        }
    ...
    async def run_human_eval(
        ...
        self.specialist_agents = self._create_specialist_agents(
            output_directory, "HumanEval"
        )

        manager_prompt = self._build_prompt(
            base_prompt=self.base_manager_prompt,
        ...
        return await self._execute_with_manager(task, manager_prompt, output_directory)
    ...
    async def _execute_with_manager(
        self, task: str, manager_prompt: str, output_directory: str
    ) -> ExecutionLog:
        """
        Execute a task using the manager agent and specialist subagents.

        Args:
            task: The task to execute
            manager_prompt: The system prompt for the manager agent
            output_directory: Working directory for the agents

        Returns:
            ExecutionLog object containing the trace and statistics
        """
        manager_instance = SubagentsManager(subagents=self.specialist_agents)
        mcp_server = self._create_communication_mcp_server(manager_instance)

        manager = self._create_manager_agent(
            manager_prompt, output_directory, mcp_server
        )

        log = ExecutionLog(debug=self.debug)
        log.set_system_prompt(manager_prompt)
        log.set_user_query(task)

        manager_instance.set_execution_log(log)

        # Spawn subagents
        await manager_instance.spawn()

        try:
            async with manager.as_client() as client:
    ...
    def _create_manager_agent(
        ...
        """Create the manager agent with communication tools only."""
        return BaseAgent(
            agent_id="Manager",
    ...
    def _process_message(self, message, log: ExecutionLog) -> None:
        """Process a message from the manager agent and update the log."""
        if isinstance(message, AssistantMessage):
            if not log.has_active_turn():
                log.start_turn(agent_id="Manager")
    ...
    def _create_specialist_agents(
        ...
        """
        Create specialist agents (PM, TL, SWE, QA).
        ...
        for agent_type, prompt in self.agent_prompts.items():
            ...
            specialist_agents[agent_type] = BaseAgent(
                agent_id=agent_type,
